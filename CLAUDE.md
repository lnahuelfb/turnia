...
**Estado**: disponibilidad end-to-end + página pública (browsing, sin confirmar reserva). Repo en GitHub (`lnahuelfb/turnia`, PR contra `main` protegida). Next.js 15.5 (App Router) + Tailwind v4, `schema.prisma` completo. Supabase (São Paulo) con schema sincronizado (`db push`), constraint anti-solapamiento y seed de demo (`/peluqueria-demo`). Clientes Supabase (browser/server/admin) + middleware de sesión. Vitest + CI (GitHub Actions). Listo: `src/lib/phone.ts`, `src/lib/availability/` (motor + capa de datos), API `GET /api/[slug]/disponibilidad`, y la **página pública `/[slug]`** (mobile-first: header + servicios + elección de profesional/día/horario + form de datos, con el submit todavía deshabilitado). Falta: la transacción de reserva (habilitar el submit), tests de integración en CI (Postgres service container), Auth (magic link + Google) + panel del comerciante, RLS para Realtime. Este archivo es la fuente de verdad del producto mientras se construye el MVP. Actualizar esta sección a medida que el proyecto avance.
...
- **"Cualquiera disponible"**: la API `GET /api/[slug]/disponibilidad` sin `profesional` devuelve la unión de slots, cada uno con `professionalIds` (quiénes están libres). La asignación concreta se define en la transacción de reserva (candidato: menos cargado ese día; TBD).
