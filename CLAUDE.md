...
**Estado**: base + tooling + disponibilidad end-to-end (sin UI). Repo en GitHub (`lnahuelfb/turnia`, se trabaja por PR contra `main` protegida). Next.js 15.5 (App Router) + Tailwind v4, `schema.prisma` completo. Supabase (São Paulo) con schema sincronizado (`db push`), constraint anti-solapamiento y seed de demo (`/peluqueria-demo`). Clientes Supabase (browser/server/admin) + middleware de sesión. Vitest + CI (GitHub Actions). Listo: `src/lib/phone.ts` (E.164 AR), `src/lib/availability/` (motor + capa de datos Prisma), y la API pública `GET /api/[slug]/disponibilidad` (verificada contra la base real). Falta: Auth (magic link + Google) end-to-end, políticas RLS para Realtime, la transacción de reserva, tests de integración en CI (service container Postgres), y toda la UI. Este archivo es la fuente de verdad del producto mientras se construye el MVP. Actualizar esta sección a medida que el proyecto avance.
