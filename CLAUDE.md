...
**Estado**: **flujo del cliente completo end-to-end** (reservar → confirmación → cancelar). Repo en GitHub (`lnahuelfb/turnia`, PR contra `main` protegida). Next.js 15.5 (App Router) + Tailwind v4, `schema.prisma` completo. Supabase (São Paulo) con schema sincronizado (`db push`), constraint anti-solapamiento y seed de demo (`/peluqueria-demo`). Clientes Supabase + middleware de sesión. Vitest (unit) + integración contra Postgres real + CI (2 jobs). Listo: `src/lib/phone.ts`, `src/lib/availability/` (motor + datos), `src/lib/booking/` (crear + cancelar + .ics + wa.me), API `GET /api/[slug]/disponibilidad`, `POST /api/[slug]/reservar`, `GET /api/turnos/[token]/ics`, `POST /api/turnos/[token]/cancelar`, la **página pública `/[slug]`** (reserva completa + pantalla de confirmación con botón wa.me + .ics + link de cancelación) y `/cancelar/[token]`. Falta: Auth (magic link + Google) + panel del comerciante (calendario, config, no-shows), notificaciones (in-app/Realtime/push/email), RLS, jobs de recordatorio, pagos (Mercado Pago), anti-ausencias. Este archivo es la fuente de verdad del producto mientras se construye el MVP. Actualizar esta sección a medida que el proyecto avance.
