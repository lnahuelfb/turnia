...
**Estado**: flujo del cliente completo + **Auth y esqueleto del panel del comerciante**. Repo en GitHub (`lnahuelfb/turnia`, PR contra `main` protegida). Next.js 15.5 (App Router) + Tailwind v4, `schema.prisma` completo. Supabase (São Paulo) con schema sincronizado (`db push`), constraint anti-solapamiento y seed de demo (`/peluqueria-demo`). Vitest (unit) + integración contra Postgres real + CI (2 jobs). Listo: cliente end-to-end (`/[slug]` reserva → confirmación wa.me/.ics → `/cancelar/[token]`), APIs de disponibilidad/reserva/ics/cancelar. **Auth**: magic link + Google OAuth vía Supabase (`/login`, `/auth/callback`, `/auth/signout`), middleware protege `/app` `/operador` `/onboarding`, espejo `public.users` sincronizado en el callback. **Panel** (`/app`): onboarding (crea comercio + suscripción TRIALING + 1 profesional), dashboard con link público + contador de turnos gratis + próximos turnos. Secciones `servicios`/`agenda`/`config` son stubs. Falta: config real de servicios/profesionales/horarios, calendario/agenda, marcar no-shows, notificaciones (in-app/Realtime/push/email), RLS, jobs de recordatorio, Mercado Pago, anti-ausencias, panel de operador. Este archivo es la fuente de verdad del producto mientras se construye el MVP. Actualizar esta sección a medida que el proyecto avance.
...
- **GitHub Actions** en cada PR y push a `main`: job `check` (`typecheck → lint → test → build`) + job `integration` (Postgres service container → `test:integration`). `main` protegida, no se mergea sin ambos en verde.
- Lint es `eslint .` (CLI directo, flat config) — ya no `next lint`.
- `server-only` se stubea en los tests (`src/test/empty.ts`) porque Vitest no tiene la resolución `react-server` de Next.
...
- **Motor de disponibilidad**: `src/lib/availability/` — función pura `computeAvailableSlots(input)`, sin dependencias de Next, con Luxon para la conversión local↔UTC (sin hardcodear offset). Entradas: horarios semanales (profesional o fallback comercio), duración + buffers del servicio, granularidad de grilla, turnos ocupados (`busy`), bloqueos/vacaciones (`blocks`), feriados (`closedDates`), `notBefore`. Salida: instantes UTC de inicio. Buffers: solo cuentan contra otros turnos, no contra el borde de la franja. El constraint `EXCLUDE` de la base tapa el solapamiento literal; los buffers se validan en la transacción de reserva. Aritmética de intervalos aislada en `interval.ts`. ~55 tests.
- **"Cualquiera disponible"**: la API `GET /api/[slug]/disponibilidad` sin `profesional` devuelve la unión de slots, cada uno con `professionalIds`. `createBooking()` re-chequea el slot por cada candidato y asigna el **menos cargado ese día** (desempata por `sortOrder`).
- **Transacción de reserva**: `src/lib/booking/create.ts`. Todo en `prisma.$transaction`: carga comercio/servicio/profesionales, chequea suscripción/cuota gratis, cliente no bloqueado, re-corre `computeAvailableSlots` para el día del slot, asigna profesional, upsert de `Client` por `(businessId, phone)`, insert de `Booking`. Si el insert choca con `booking_no_overlap` (carrera perdida) → `BookingError("SLOT_TAKEN")`. Cuota gratis: se incrementa `freeBookingsUsed` solo si la suscripción es `TRIALING` o no existe.
- **Tests de integración**: `*.integration.test.ts`, corren con `npm run test:integration` (config aparte, sin paralelismo) contra Postgres real. En CI: job `integration` con service container `postgres:17` (`db push` + `db:sql` + tests). Local: `docker run -p 5433:5432 postgres:17` + `DATABASE_URL` apuntando ahí. Helpers en `src/test/db.ts` (`resetDb`, `createTestBusiness`).
- **Normalización de teléfono**: `normalizeArPhone()` en `src/lib/phone.ts` (usa `libphonenumber-js`). Guardar siempre en E.164 (`+549...`). Un número AR de 10 dígitos sin marca de celular se asume celular.
- **Slugs**: `src/lib/slug.ts` — `slugify()` (saca acentos/espacios), `validateSlug()` (formato + `RESERVED_SLUGS`). Mantener `RESERVED_SLUGS` en sync con los segmentos de ruta reales. Pendiente: si el comercio cambia su slug, dejar redirect del viejo.
- **Auth**: `src/lib/auth.ts` — `getUser()` / `requireUser()` (Supabase), `getMyBusiness(userId)` (Prisma, envuelto en `cache()`), `syncUserRow()` (upsert de `public.users` desde la sesión, se llama en `/auth/callback`). La autorización de datos del panel es en código server (Prisma bypasea RLS); RLS recién importa para Realtime. Un usuario = un comercio (`Business.ownerId @unique`).
...
- **Cancelación**: `src/lib/booking/cancel.ts`. El link (`/cancelar/[token]`) usa `Booking.cancelToken` (opaco, no el id). Idempotente: si ya está cancelado devuelve `already_cancelled` sin error. Al cancelar, el estado sale de los "activos" → el slot se libera solo (motor + constraint filtran por estado). MVP: no descuenta cuota gratis ni aplica política de cancelación tardía (parte del anti-ausencias, TBD).
- **`.ics` y `wa.me`**: `src/lib/booking/ics.ts` (VCALENDAR con VALARM T-2h, CRLF, escape) y `src/lib/booking/whatsapp.ts` (puros, client-safe). El `.ics` se sirve desde `GET /api/turnos/[token]/ics` como `text/calendar`. El botón `wa.me` arma el mensaje cliente→comercio en la pantalla de confirmación.
