...
**Estado**: reserva pública end-to-end en el backend (falta enganchar el submit del form). Repo en GitHub (`lnahuelfb/turnia`, PR contra `main` protegida). Next.js 15.5 (App Router) + Tailwind v4, `schema.prisma` completo. Supabase (São Paulo) con schema sincronizado (`db push`), constraint anti-solapamiento y seed de demo (`/peluqueria-demo`). Clientes Supabase + middleware de sesión. Vitest (unit) + tests de integración contra Postgres real + CI (GitHub Actions, 2 jobs). Listo: `src/lib/phone.ts`, `src/lib/availability/` (motor + capa de datos), `src/lib/booking/create.ts` (transacción de reserva), API `GET /api/[slug]/disponibilidad` y `POST /api/[slug]/reservar`, y la **página pública `/[slug]`** (mobile-first, hasta elegir horario + form; el submit todavía no llama a la API). Falta: enganchar el submit + pantalla de confirmación (wa.me + .ics), Auth (magic link + Google) + panel del comerciante, RLS para Realtime, notificaciones. Este archivo es la fuente de verdad del producto mientras se construye el MVP. Actualizar esta sección a medida que el proyecto avance.
