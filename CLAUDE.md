...
**Estado**: base + tooling + motor de disponibilidad. Repo en GitHub (`lnahuelfb/turnia`, se trabaja por PR contra `main` protegida). Next.js 15.5 (App Router) + Tailwind v4, `schema.prisma` completo. Supabase (São Paulo) con schema sincronizado (`db push`), constraint anti-solapamiento y seed de demo (`/peluqueria-demo`). Clientes Supabase (browser/server/admin) + middleware de sesión. Vitest + CI (GitHub Actions). Lógica pura lista: `src/lib/phone.ts` (E.164 AR) y `src/lib/availability/` (motor de disponibilidad). Falta: Auth (magic link + Google) end-to-end, políticas RLS para Realtime, la capa que arma el input del motor desde la base, la transacción de reserva, y la UI. Este archivo es la fuente de verdad del producto mientras se construye el MVP. Actualizar esta sección a medida que el proyecto avance.
